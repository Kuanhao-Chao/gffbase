...
def sample_gene_ids(n: int, seed: int = 20260501, *, threads: int = 1):
...
def run_gffbase(gene_ids, *, force_dynamic: bool = False, threads: int = 1):
    import gffbase

    db = gffbase.FeatureDB(str(GFFBASE_DB), read_only=True)
...
def main():
    ...
    gene_ids = sample_gene_ids(args.n_genes, threads=args.threads)
    print(f"  sampled {len(gene_ids)} genes", flush=True)

    print("[relational] gffbase auto-routed (closure cache by default)…", flush=True)
    g_auto_wall, g_auto_total = run_gffbase(gene_ids, force_dynamic=False, threads=args.threads)
    print(
    ...
    g_dyn_wall, g_dyn_total = run_gffbase(gene_ids, force_dynamic=True, threads=args.threads)
    print(
