...
For per-method snippets see the [Usage gallery](usage_gallery.md); for
corpus-specific workflows see the [Cookbooks](cookbooks/index.md).
...
like [Splam](https://khchao.com/splam/) or
[OpenSpliceAI](https://khchao.com/OpenSpliceAI/) trains on. An intron is the
...
!!! tip "At whole-genome scale, batch it"
...
representative transcript per gene — see [MANE](cookbooks/mane.md).
...
[`to_table()`](api/featuredb.md) takes the same filters as `all_features()` and
...
built. See [Command line](cli.md).

---

## Where to go next

- **[Cookbooks](cookbooks/index.md)** — the same techniques against GENCODE,
  RefSeq and MANE, at whole-genome scale.
- **[Machine learning workflows](cookbooks/machine_learning_workflows.md)** —
  bulk extraction into PyTorch and Hugging Face with no per-row Python.
- **[Usage gallery](usage_gallery.md)** — one snippet per public method.
