...
migration is one import change. **However**, there is one important
behavioral gotcha that almost all ML and bulk-pipeline users will hit
unless they explicitly migrate to the new vectorized API. Read §3
carefully.
...
| Metric | Speedup |
|---|---|
| GENCODE-scale ingest (`create_db`) | **15.87×** |
| Spatial overlap queries (`db.region(...)`) | **5.20×** (8.3× lower p50 latency) |
| `db.children(id, level=1)` indexed lookup | comparable |

Your existing `gffutils` script will get the ingest and spatial wins
the moment you swap the import.

---

## 3. ⚠️ The OLAP vs OLTP tradeoff — the **one important migration**
...
```python
# ❌ ANTI-PATTERN with gffbase: this iterates 50 000 small queries.
# Pays DuckDB's vectorization startup × 50 000 + per-row Feature object
# construction × 1.6 M. Takes 10+ minutes on GENCODE.
...
…then on migration day you will see your script get **slower**, not
faster, even though every other workload accelerated. This is a real
limitation, not a bug, and **it has a one-line fix**:

```python
# ✅ The vectorized API. ONE SQL query for all 50 000 transcripts.
# Returns a zero-copy pyarrow.Table — no Feature object ever
# constructed. 1.16 s wall on GENCODE v45.
...
# The `anchor` column carries the input transcript_id for each row, so
# you can groupby in Python or downstream Arrow tooling without
# re-issuing N queries:
import pyarrow.compute as pc
exon_count_per_tx = pc.value_counts(exons.column("anchor"))
```

Full benchmark (`PERFORMANCE_COMPARISON.md` §4b): at 50 000
transcripts the row-by-row loop is ≥10 minutes; the batched call is
**1.16 s — a 553× speedup**.
...
- Small annotations (< 100k features) where SQL startup overhead is
  not visible.
...
- **Peak ingest RSS**: ~10× higher (1.45 GB vs 145 MB on GENCODE
